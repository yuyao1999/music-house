import { _support, validateOption, isBrowserEnv, Queue, isEmpty, getLocationHref, generateUUID } from '@/websee/utils'
...
export class TransportData {
  ...
  // 驼峰转下划线
  toLine(data: any): object {
    const toLine = (str: string) => {
      return str.replace(/([A-Z])/g, '_$1').toLowerCase()
    }
    const res = {} as any
    for (const key in data) {
      res[toLine(key)] = data[key]
    }
    return res
  }

  beacon(url: string, data: any): boolean {
    return navigator.sendBeacon(url, JSON.stringify(this.toLine(data)))
  }
  ...
  async xhrPost(data: ReportData, url: string): Promise<void> {
    const requestFun = () => {
      fetch(`${url}`, {
        method: 'POST',
        body: JSON.stringify(this.toLine(data)),
        headers: {
